...
$(document).on('click', '#add-task', function (e) {
  ...
  var newTask = $('#task_description').val();
  var allTaskUrl = $(this).data('create');
  $.ajax({
    url: allTaskUrl,
    type: 'POST',
    data: {
    ...
    success: function success(response) {
      ...
      var list = $('.list');
      if (list.length === 0) {
        var createBlock = $('#create_task');
        createBlock.after("\n                        <div class=\"mb-4\" id=\"all-tasks\">\n                            <h1>All Tasks</h1>\n                            <div id=\"task-list\">\n                                <ul class=\"list\">\n                                    <li class=\"card\">\n                                        <div class=\"card-body\">\n                                            <p>".concat(response.description, "</p>\n                                            <input type=\"hidden\" name=\"id\" value=\"").concat(response.id, "\">\n                                            <button class=\"btn btn-secondary\" type=\"submit\" data-update=\"/\">Complete</button>\n                                        </div>\n                                    </li>\n                                </ul>\n                            </div>\n                        </div>\n                    "));
      }
      ;
      var html = "\n                        <li class=\"card ".concat(response.completed ? 'border-success' : '', "\">\n                        <div class=\"card-body\">\n                            <p>").concat(response.description, "</p>\n                            <input type=\"hidden\" name=\"id\" value=\"").concat(response.id, "\">\n                            ").concat(response.completed ? "<button class=\"btn btn-danger\" type=\"submit\" data-delete=\"/\">Delete</button>" : "<button class=\"btn btn-secondary\" type=\"submit\" data-update=\"/\">Complete</button>", "\n                        </div>\n                    </li>\n                ");
      var completedTask = $('.border-success').last();
...
$(document).on('click', '.btn-secondary', function (e) {
  e.preventDefault();
  var csrfToken = $('meta[name="csrf-token"]').attr('content');
  var taskId = $(this).prev('input').val();
  var updateTaskUrl = $(this).data('update');
  $.ajax({
    url: updateTaskUrl,
    type: 'PATCH',
    data: {
    ...
    success: function success(response) {
      var input = $('input[value="' + response.id + '"]');
      input.next('.btn-secondary').remove();
      input.parents('.card').addClass('border-success');
      var deleteButton = "<button class=\"btn btn-danger\" type=\"submit\" data-delete=\"/\">Delete</button>";
...
$(document).on('click', '.btn-danger', function (e) {
  ...
  var csrfToken = $('meta[name="csrf-token"]').attr('content');
  var taskId = $(this).prev('input').val();
  var deleteTaskUrl = $(this).data('delete');
  $.ajax({
    url: deleteTaskUrl,
    type: 'DELETE',
    data: {
